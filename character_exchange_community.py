...
for filename in os.listdir(input_directory):
    ...
    if interaction_matrix.shape[0] != interaction_matrix.shape[1]:
        print(f"Matrix in {filename} is not square! Shape: {interaction_matrix.shape}. Skipping this file.")
        continue
    ...
    fig = go.Figure(data=[edge_trace, node_trace],
             layout=go.Layout(
                title=f'Community Detection for {os.path.splitext(filename)[0]}',
                titlefont_size=16,
                showlegend=False,
                hovermode='closest',
    ...
    fig.write_html(os.path.join('community_detection', f"{os.path.splitext(filename)[0]}_community.html"))
    ...
    #             colorscale='Electric'))
    heatmap_fig.write_html(os.path.join('heatmaps', f"{os.path.splitext(filename)[0]}_heatmap.html"))
